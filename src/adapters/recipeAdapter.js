export default class RecipeAdapter {
	...
	static saveRecipe(recipeParams) {
		...
		return fetch('https://brewforyouapi.herokuapp.com/api/v1/recipes', {
			method: 'POST',
			body: recipeJSON,
			headers: {
				'Authorization': token,
	...
	static getFromBackend(id) {
		const token = localStorage.getItem('jwt')
		return fetch(`https://brewforyouapi.herokuapp.com/api/v1/recipe/${id}`, {
			method: 'GET',
			headers: {
				'Authorization': token,
				'Content-Type': 'application/json',
	...
	static saveReview(review, id) {
		...
		const token = localStorage.getItem("jwt")
		return fetch('https://brewforyouapi.herokuapp.com/api/v1/review/new', {
			method: 'POST',
			body: reviewJSON,
			headers: {
				'Authorization': token,
